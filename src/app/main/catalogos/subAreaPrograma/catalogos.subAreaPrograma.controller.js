(function ()
{
    ...
    angular
        .module('app.catalogos.subAreaPrograma')
        .controller('subAreaProgramaController', subAreaProgramaController);

    /** @ngInject */
    function subAreaProgramaController($scope, $mdDialog, localStorageService,$state) {
        var vm = this;
        ...
        vm.agregarSubAreaPrograma = agregarSubAreaPrograma;
        vm.editarSubAreaPrograma = editarSubAreaPrograma;
        vm.eliminarSubAreaPrograma = eliminarSubAreaPrograma;

        function eliminarSubAreaPrograma(modalidad){
            AjaxCall("POST", URL_SERVIDOR + "/ProgramSubarea/delete/"+modalidad.id, null, function (response) {
                StateMessage("Atención","Sub-Area eliminada correctamentre","success");
                vm.init();
            });
        }

        function agregarSubAreaPrograma(ev, data)
        {
            ...
            $mdDialog.show({
                controller: function ($scope, $mdDialog, formWizardData)
                {
                    ...
                    {
                        AjaxCall("POST",URL_SERVIDOR+ '/ProgramSubarea',
                ...
                templateUrl : 'app/main/catalogos/subAreaPrograma/html/formularioSubArea.html',
        ...
        function editarSubAreaPrograma(ev, data)
        {
            // You can do an API call here to send the form to your server
            ...
            $mdDialog.show({
                controller: function ($scope, $mdDialog, formWizardData)
                {
                    ...
                    $scope.enviarInformacion= function ()
                    {
                        AjaxCall("POST",URL_SERVIDOR+ '/ProgramSubarea/update/'+ $scope.data.id,
                            $scope.data,function(response){
                              //  if(response.Codigo == "201"){
                                    StateMessage("Atención", "Sub Area modificada correctamente", "success");
                                    $scope.closeDialog();
                                    vm.init();
                            //    }
                            });
        ...
        function init(){
            ...
            AjaxCall("GET", URL_SERVIDOR + "/ProgramSubarea", null, function (response) {
                ...
                $scope.$apply(function(){
                    $scope.subAreaPrograma = response.datos;
                });
